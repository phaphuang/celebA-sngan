...
def get_args():
    ...
    parser.add_argument('--data_root', type=str, default='data/CelebA',
                        help='path to dataset root directory. default: CelebA')
...
def main():
    ...
    train_dataset = datasets.ImageFolder(
        os.path.join(args.data_root, 'resized_celebA_identity/train'),
        transforms.Compose([
    ...
    if args.calc_FID:
        eval_dataset = datasets.ImageFolder(
            os.path.join(args.data_root, 'splits/val'),
            transforms.Compose([
    ...
    if torch.cuda.device_count() > 1:
        print("Let's use generator with", torch.cuda.device_count(), "GPUs!")
    ...
    if args.dis_arch_concat:
        ...
        if torch.cuda.device_count() > 1:
            print("Let's use SNResNetConcat discriminator with", torch.cuda.device_count(), "GPUs!")
        ...
        if torch.cuda.device_count() > 1:
            print("Let's use SNResNetProjection discriminator with", torch.cuda.device_count(), "GPUs!")
            dis = nn.DataParallel(dis)
        dis.to(device)
